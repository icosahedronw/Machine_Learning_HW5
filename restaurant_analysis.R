...
str(all_data)   #375012 obs, 18 features


#A)
#a)
#i) Drop additional following columns
all_data<-all_data %>% dplyr:: select(-BUILDING, -STREET, -PHONE, -DBA,-ZIPCODE, -12, -16, -17)
# ii) Make INSPECTION DATE a date object
...
# We have 355359 obs and 25464 unique restaurants
dim(all_data)
length(unique(all_data$id))

all_data= all_data %>% group_by(id) %>% mutate(max_score=max(score)) %>% dplyr::ungroup()
...
restaurant_data1<-restaurant.data%>% dplyr::select(id,borough,cuisine,inspection_year,inspection_date,score)
...
restaurant_data<-restaurant_data2%>% dplyr::select(-id,-score,-inspection_date)
...
all_data_2<-all_data%>% dplyr::select(id,score,action,inspection_date)
all_data_2$inspection_year<-year(all_data_2$inspection_date)
View(all_data_2)


unique(all_data_2$inspection_year)
all_data_2= all_data_2[all_data_2$inspection_year!=2018, ] #258326 obs
...
joined_restaurant_2 <- joined_restaurant_2 %>% dplyr::select(id,outcome,borough,cuisine,inspection_date,inspection_year.x,month,weekday,num_previous_low_inspections,num_previous_med_inspections,num_previous_high_inspections,num_previous_closings)
#Drop duplicated obs
...
arrange(desc(n)) %>%
slice(1:50)
restaurant_data<-joined_restaurant_2%>% dplyr::filter(joined_restaurant_2$cuisine%in%top50$cuisine) #108082 obs
unique(restaurant_data$cuisine) #38363 obs
restaurant_data<- dplyr::rename(restaurant_data, inspection_year = inspection_year.x)
restaurant_data <-restaurant_data %>% dplyr::select(outcome,borough,cuisine,inspection_year,month,weekday,num_previous_low_inspections,num_previous_med_inspections,num_previous_high_inspections,num_previous_closings)
...
View(restaurant_data)
#Create training set
train<-restaurant_data%>% dplyr::filter(inspection_year==2015|inspection_year==2016)
#Create testing set
test<-restaurant_data%>% dplyr:: filter(inspection_year==2017)
